...
{
	$sql = "SELECT * FROM peminjaman INNER JOIN anggota ON peminjaman.id_anggota = anggota.id_anggota INNER JOIN buku_perpus ON peminjaman.id_buku = buku.id_buku WHERE id_pinjam = $id_pinjam";
	$res = mysqli_query($kon, $sql);
...
{
	$sql = "UPDATE buku SET stok = $stok_update WHERE id_buku";
	$res = mysqli_query($kon, $sql);
